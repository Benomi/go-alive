...
```console
foo@bar:~$ go install github.com/Benomi/go-alive@latest
```

If you are using an older version of golang,
```console
foo@bar:~$ GO111MODULE=on go get github.com/Benomi/go-alive@latest
```
## Using Docker ##
...
```console
foo@bar:~$ git@github.com:Benomi/go-alive.git && cd go-alive
```
...
```console
foo@bar:~$ docker pull benomiteklef/go-alive
```
